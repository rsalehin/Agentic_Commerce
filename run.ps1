...
if ((Test-Path (Join-Path $uiDir "package.json")) -and $npm) {
    if (-not (Test-Path (Join-Path $uiDir "node_modules"))) {
        ...
        Start-Process -FilePath $npm.Source -ArgumentList @("install") -WorkingDirectory $uiDir -Wait
    }
    Write-Host ("Starting ui on port {0} (Vite) ..." -f $uiPort)
    Start-Process -FilePath $npm.Source -ArgumentList @("run", "dev") -WorkingDirectory $uiDir | Out-Null
    $uiStarted = $true
}
else {
